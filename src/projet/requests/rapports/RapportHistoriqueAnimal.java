    ...
    public void afficherDossier(int idAnimal) {
        ...
        try (Connection conn = Connexion.connectR();
            ...
            try (ResultSet rs = pstInc.executeQuery()) {
                ...
                if (empty) System.out.println("R.A.S (Aucun incident).");
            }
            ...
            try (ResultSet rs = pstBox.executeQuery()) {
                ...
                while (rs.next()) {
                    empty = false;
                    String fin = (rs.getDate("DATE_F_BOX") != null) ? rs.getDate("DATE_F_BOX").toString() : "ACTUELLEMENT";
                    System.out.printf("- Box %s : du %s au %s%n", 
                        rs.getString("type_box"),
                        rs.getDate("DATE_D"),
                        fin);
                }
                if (empty) System.out.println("Aucun historique de box.");
            }
